...
public class Client extends Thread {
	...
	public void run() {
		try {
			List<Account> accounts = new ArrayList<Account>(3);
			...
			List<Account> tlist = new ArrayList<Account>(accounts);
			accounts.clear();
		 	for (Account e : tlist) 
					accounts.add((Account)transaction.accesses(e));	
			...
			for (Account account : accounts) {
				System.out.println(id + " waits for access to object");
				String name = account.getName();
				System.out.println(id + " gained access to " + name);				
				if (name.equals("C")) {
					System.out.println(id + " " + name + ": " + account.getBalance());
					Thread.sleep(5000);
					account.deposit(100);
				} else {
