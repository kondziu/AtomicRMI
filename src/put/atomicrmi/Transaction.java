...
public class Transaction extends UnicastRemoteObject implements ITransaction {
	...
	public <T> T writes(T obj, int calls) throws TransactionException {
		return accesses(obj, calls, calls, Mode.WRITE_ONLY);
	}

	@SuppressWarnings("unchecked")
	public <T> T accesses(T obj, long allCalls, long writes) throws TransactionException {
		...
		try {
			...
			IObjectProxy proxy = (IObjectProxy) remote.createProxy(this, id, allCalls, writes, Mode.ANY);
			proxies.add(proxy);
